...
function Message({ openChat, setOpenChat, msgs, setMsg, msg, SendMessage, user, roomTitle}) {
    return (
        <>
            <div class={`fixed  bottom-0 right-0 flex flex-col items-end ml-6 w-full mb-4 ${!openChat && "chat-hide"} `} >
                <div class={`chat-modal ${openChat && "show"}   mr-5 flex flex-col mb-5 shadow-lg sm:w-1/2 md:w-1/3 lg:w-1/4`}>
                    <div class="close-chat bg-red-500 hover:bg-red-600 text-white mb-1 w-10 flex justify-center items-center px-2 py-1 rounded self-end cursor-pointer" onClick={() => setOpenChat(false)}>
                    ...
                    <div class="flex justify-between items-center text-white p-2 bg-green-500 border shadow-lg mr-5 w-full">
                        <div class="flex items-center">
                            ...
                            <h2 class="font-semibold tracking-wider">{roomTitle}</h2>
                        </div>
